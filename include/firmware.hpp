...
struct pac_header_t {
    ...
    uint32_t nFileCount;         // the number of files that will be downloaded, the
                                 // file may be an operation
    uint32_t dwFileOffset;       // the offset from the packet file header to the
                                 // array of FILE_T struct buffer
    uint32_t dwMode;
...
struct bin_header_t {
    ...
    uint16_t szFileName[256];     // file name,in the packet bin file,it only
                                  // stores file name but after unpacketing, it
                                  // stores the full path of bin file
    ...
    uint32_t nFileFlag;           // if "0", means that it need not a file, and
                                  // it is only an operation or a list of operations, such as
                                  // file ID is "FLASH" if "1", means that it need a file
    uint32_t nCheckFlag;          // if "1", this file must be downloaded;
                                  // if "0", this file can not be downloaded;
    uint32_t dwLoDataOffset;      // the offset from the packet file header to this
                                  // file data
    uint32_t dwCanOmitFlag;       // if "1", this file can not be downloaded and not
                                  // check it as "All files"
